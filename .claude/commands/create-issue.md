...
- Clear title
- Issue ID -> auto-increment from the highest existing ID (ID-###)
...
## Output Format
- Use a `<details>` block per issue with this summary format:
  - `<summary><strong>ID</strong> | Type | [Priority Tag] | Name</summary>`
- Priority tags:
  - `🔥` = high, `⚡` = normal, `🧊` = low
- Inside the details block, include bullets in this order:
  - **TL;DR:** ...
  - **Current:** ...
  - **Expected:** ...
  - **Likely files:** ...
  - **Risk/notes:** ...
  - **Labels:** type: ___, priority: ___, effort: ___
- Add a blank line before and after the details block for readability.

## Output Location
- Document the issue in the `## Open Issues` section of the `_docs/issue-backlog.md` file
- Never move items out of Issue Backlog unless the work is fully completed and verified.
